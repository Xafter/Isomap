...
def SIsomap(data, K, d):
    G = select_k.get_Graph(data, K)
    N = data.shape[0]
    G_ = G.copy()
    for i in range(N):
...
if __name__ == "__main__":
    ...
    Y1 = SIsomap(data, K, d)
    Y2 = Isomap(data, K, d)
    sio.savemat(data_name + '.mat', {"Y": Y1})
    fig = plt.figure(1)
    plt.scatter(Y1[:, 0], Y1[:, 1], c=color, cmap=plt.cm.jet)
    fig  = plt.figure(2)
    plt.scatter(Y2[:, 0], Y2[:, 1], c=color, cmap=plt.cm.jet)
    plt.show()
    '''
    path = './mnist_data'
    ...
    for d in ds:
        ...
        if not os.path.exists(save_path):
            os.mkdir(save_path)
        sio.savemat(save_path + r"/" + str(d) + 'D_I' + data_name, {'Y': Y})
    print("complete")
    #face()